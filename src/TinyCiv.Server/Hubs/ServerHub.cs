...
public class ServerHub : Hub
{
    ...
    public override async Task OnDisconnectedAsync(Exception? exception)
    {
        ...
        try
        {
            _logger.LogInformation("{method_name}: initiating leave lobby event for '{connection_id}'", nameof(OnDisconnectedAsync), _accessor.ConnectionId);
            
            await handler
                .HandleAsync(Clients.Caller, Clients.All, JsonSerializer.Serialize(ungracefulLeaveLobbyEvent))
                .ConfigureAwait(false);
            
            _logger.LogInformation("{method_name}: successfully left lobby '{connection_id}'", nameof(OnDisconnectedAsync), _accessor.ConnectionId);
        }
        catch (Exception e)
        {
